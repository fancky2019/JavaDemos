 ...
 *程序计数器是为一个一个不会出现OutOfMemoryError的内存区域，他的生命周期随着线程的创建而创建，随着现成的结束而死亡。
 *
 *
 * java 内存区域(运行时数据区)、java 内存模型 JMM
 *
 *
 *
 * jdk8中字符串常量池存放在堆中
 * string、static:java heap 中
 ...
 * 在jdk1.8中，永久代已经不存在，存储的类信息、编译后的代码数据等已经移动到了元空间(MetaSpace 堆外内存--本地内存)中，
 * 元空间并没有处于堆内存上，而是直接占用的本地内存(NativeMemory)。
 *
 * 元空间：类、字段、方法的修饰信息，保存在本地内存中。
 *
