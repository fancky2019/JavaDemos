...
import Test.opensource.Netty.mqtt.client.TimerData;
import Test.opensource.Netty.mqtt.protocol.MqttNettyServerHandler;
 ...
 *
 *  实现QoS>0的MQTT通讯，客户端在连接服务端时必须要将cleanSession设置为false。
 *
...
@Slf4j
//@Component
public class MqttNettyMsgBacServer {
    ...
    private MqttNettyMsgBacServer()
    {

    }
    private static class StaticInnerClass{
        private static final MqttNettyMsgBacServer instance=new MqttNettyMsgBacServer();
    }

    public static MqttNettyMsgBacServer getInstance() {
    ...
    public void publishAck(ChannelHandlerContext ctx, MqttMessage mqttMessage) {
        ...
        if (set != null) {
            for (String channelId : set) {
                ChannelHandlerContext context = MqttNettyServerHandler.clientMap.get(channelId);
        ...
        switch (qos) {
            ...
            case AT_LEAST_ONCE:
                ...
                //构建返回报文， 固定报头
                MqttFixedHeader mqttFixedHeaderBack = new MqttFixedHeader(MqttMessageType.PUBACK, mqttFixedHeaderInfo.isDup(), AT_MOST_ONCE, mqttFixedHeaderInfo.isRetain(), 0x02);
                //构建PUBACK消息体
    ...
    public void receivePubAck(ChannelHandlerContext ctx, MqttMessage mqttMessage) {
        ...
        if (messageType == MqttMessageType.PUBACK) {
        ...
        if (messageType == MqttMessageType.PUBREC) {
